package com.jwding.appbase.component.lockview;
...
import com.jwding.appbase.component.nineoldanim.animation.ObjectAnimator;
import com.topxgun.appbase.R;
import com.jwding.appbase.component.autolayout.utils.AutoUtils;
import com.jwding.appbase.component.nineoldanim.animation.Animator;
import com.jwding.appbase.component.nineoldanim.view.ViewHelper;
import com.jwding.appbase.util.DensityUtil;
...
public class CustomSlideToUnlockView extends RelativeLayout {
    ...
    private static float  THRESHOLD = 0.9F;//滑动阈值比例:默认是0.5,即滑动超过父容器宽度的一半再松手就会触发
    ...
    private void init(TypedArray mTypedArray) {
        ...
        slideImageViewWidth= AutoUtils.getPercentWidthSize(slideImageViewWidth);
    ...
    protected void initView() {
        ...
        RelativeLayout.LayoutParams lp= (LayoutParams) rl_root.getLayoutParams();
        lp.setMargins(slideImageViewWidth/2,slideImageViewWidth/8,slideImageViewWidth/2,slideImageViewWidth/8);
        ...
        tv_hint.setText(TextUtils.isEmpty(textHint)? mContext.getString(R.string.comfirm):textHint);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch (event.getAction()) {
                    ...
                    case MotionEvent.ACTION_MOVE://上滑才处理,如果用户一开始就下滑,则过掉不处理
                        ...
                        if (mSlidedDistance > 0&&leftNew<AutoUtils.getPercentWidthSize(800)-slideImageViewWidth) {
                            //直接通过margin实现滑动
...
//                        }
    ...
    private void scrollToLeft(final View v) {
        ...
        ObjectAnimator.ofFloat(rl_slide,"TranslationX",ViewHelper.getTranslationX(v), -params1.leftMargin).setInterpolator(new AccelerateInterpolator()).setDuration(DEAFULT_DURATIN_LONG).addListener(new Animator.AnimatorListener() {
            @Override
    ...
    private void scrollToRight(final View v) {
        ...
        ObjectAnimator.ofFloat(rl_slide,"TranslationX",ViewHelper.getTranslationX(v), ( rl_slide.getWidth() - params1.leftMargin-slideImageViewWidth)).setDuration(DEAFULT_DURATIN_SHORT).setInterpolator(new AccelerateInterpolator()).addListener(new Animator.AnimatorListener() {
            @Override
            public void onAnimationStart(Animator animation) {
