...
/**
 * Created by rjm4413 on 2016/12/26.
 */
...
//	    return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
    ...
    public static Bitmap compressPicture(Bitmap srcBitmap,int maxWidth) {
        BitmapFactory.Options op = new BitmapFactory.Options();
        // 缩放图片的尺寸
        float w = srcBitmap.getWidth();
        float h = srcBitmap.getHeight();
        float hh = maxWidth;//
        float ww = maxWidth;//
        // 最长宽度或高度1024
        float be = 1.0f;
        ...
        int desWidth = (int) (w / be);
        int desHeight = (int) (h / be);
        Bitmap bitmap = Bitmap.createScaledBitmap(srcBitmap, desWidth, desHeight, true);
        return bitmap;
    }
